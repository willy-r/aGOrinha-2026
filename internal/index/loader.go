...
func loadBinaryIndex(idx *Index, path string) error {
	...
	// Vectors: numVecs × Dims int16 — read directly into slice bytes
	idx.Vecs = make([][Dims]int16, numVecs)
	vecsBytes := unsafe.Slice((*byte)(unsafe.Pointer(&idx.Vecs[0])), int(numVecs)*Dims*2)
	if _, err := io.ReadFull(f, vecsBytes); err != nil {
		return fmt.Errorf("reading vectors: %w", err)
	}

	// Labels: numVecs uint8
	rawLabels := make([]uint8, numVecs)
	if _, err := io.ReadFull(f, rawLabels); err != nil {
		return fmt.Errorf("reading labels: %w", err)
	}
	idx.Labels = make([]bool, numVecs)
